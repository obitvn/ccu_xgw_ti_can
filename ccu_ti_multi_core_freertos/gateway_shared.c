...
void gateway_core0_ipc_callback(uint16_t clientId, uint16_t msg)
{
    switch (msg) {
        ...
        case MSG_TEST_DATA_READY:
            /* Test data from Core 0 to Core 1 - not used on Core 0 */
            gGatewaySharedMem.stats.ipc_notify_count[clientId & 0x3]++;
            break;

        case MSG_TEST_DATA_FROM_CORE1:
            /* Test data from Core 1 - acknowledged */
            gGatewaySharedMem.stats.ipc_notify_count[clientId & 0x3]++;
            break;
...
int gateway_notify_test_data_ready(void)
{
    ...
    /* Since this is in Core 0's binary, we ARE Core 0, send to Core 1 */
    uint32_t remote_core_id = CSL_CORE_ID_R5FSS0_1;

    DebugP_log("[IPC] Test data notification: Core 0 -> Core 1, msgValue=0x%02X\r\n", MSG_TEST_DATA_READY);

    int32_t status = IpcNotify_sendMsg(
        remote_core_id,
        GATEWAY_IPC_CLIENT_ID,
        MSG_TEST_DATA_READY,  /* Now using 0x09 instead of 0x10 */
        1
    );

    if (status == SystemP_SUCCESS) {
        DebugP_log("[IPC] Test data notification: SUCCESS\r\n");
    } else {
        DebugP_log("[IPC] Test data notification: FAILED! status=%d\r\n", status);
    }
